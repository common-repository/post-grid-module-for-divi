...
trait Field_Processor {
	 ...
	 * @since 1.0.0
	 */
	public function squad_generate_additional_styles( $options = array() ) {
		...
		$css_prop          = self::field_to_css_prop( isset( $options['css_property'] ) ? $options['css_property'] : $options['type'] );
		$last_modified_key = sprintf( '%1$s_last_edited', $qualified_name );

		list( $value_default, $value_last_edited, $value_responsive_values ) = $this->squad_collect_prop_value_responsive( $options, $qualified_name, $last_modified_key );

		if ( et_pb_get_responsive_status( $value_last_edited ) && '' !== implode( '', $value_responsive_values ) ) {
			$this->squad_generate_responsive_styles(
				array(
		...
		if ( isset( $options['hover'] ) && '' !== $width_hover ) {
			...
			$this->set_style( $this->slug, $hover_style );
		}
	}
	 ...
	 * @return array
	 */
	public function squad_collect_prop_value_responsive( $options, $qualified_name, $last_modified_key ) {
	 ...
	 * @return void
	 */
	protected function squad_generate_responsive_styles( $options ) {
	 ...
	 * @return mixed
	 */
	protected function squad_collect_prop_mapping_value( $options, $current_value ) {
	 ...
	 * @since 1.0.0
	 */
	protected function squad_generate_margin_padding_styles( $options = array() ) {
		// Initiate default values for current options.
		$default = array(
			'field'          => '',
			'selector'       => '',
		...
		if ( et_pb_get_responsive_status( $value_last_edited ) && '' !== implode( '', $value_responsive_values ) ) {
			$value_responsive_values = array_map(
				function ( $current_value ) use ( $options ) {
					return $this->squad_collect_prop_mapping_value( $options, $current_value );
				},
			...
			self::set_style(
				...
				array(
					...
					'declaration' => et_builder_get_element_style_css(
						esc_html( $this->squad_collect_prop_mapping_value( $options, $value_default ) ),
						$css_prop,
						$options['important']
					),
				)
		...
		if ( isset( $hover_selector ) && '' !== $margin_padding_hover ) {
			$hover_style = array(
				...
				'declaration' => et_builder_get_element_style_css(
					esc_html( $this->squad_collect_prop_mapping_value( $options, $margin_padding_hover ) ),
					$css_prop,
					$options['important']
				),
			);
	 ...
	 * @since 1.0.0
	 */
	public function squad_generate_show_icon_on_hover_styles( $options = array() ) {
		$additional_css = '';
		...
		$css_prop = self::field_to_css_prop(
		...
		$icon_width_values = $this->squad_get_icon_hover_effect_prop_width(
			$module_props,
			array(
		...
		foreach ( $icon_width_values as $device => $current_value ) {
			...
			$css_value = $this->squad_hover_effect_generate_css(
				$module_props,
				array(
			...
			if ( 'desktop' !== $device ) {
				$current_media_query  = 'tablet' === $device ? 'max_width_980' : 'max_width_767';
				$style['media_query'] = $this->get_media_query( $current_media_query );
			}
	 ...
	 * @since 1.0.0
	 */
	private function squad_get_icon_hover_effect_prop_width( $props, $options = array() ) {
		$defaults      = array(
			'icon'   => '',
			'image'  => '',
			'lottie' => '',
	 ...
	 * @since 1.0.0
	 */
	private function squad_hover_effect_generate_css( $props, $options = array() ) {
		// Initiate default values for current options.
		$default_options = array(
			'qualified_name'     => '',
			'mapping_values'     => array(),
		...
		$field_value          = $this->squad_collect_prop_mapping_value( $options, $default_value );
